...
export function useSkeets(options = {}) {
  ...
  useEffect(() => {
    ...
    const controller = createPollingController({
      fetchBluesky: async () => {
        ...
        if (!res.ok) {
          ...
          if (res.status === 429 || res.status === 503) {
            ...
            const err = new Error(data.error || "Rate Limit erreicht – später erneut versuchen.");
      ...
      activeIntervalMs: parseMs(clientConfig?.polling?.skeets?.activeMs, DEFAULTS.skeets.activeMs),
      idleIntervalMs: parseMs(clientConfig?.polling?.skeets?.idleMs, DEFAULTS.skeets.idleMs),
      hiddenIntervalMs: parseMs(clientConfig?.polling?.skeets?.hiddenMs, DEFAULTS.skeets.hiddenMs),
      minimalPingWhenHidden: parseBool(clientConfig?.polling?.skeets?.minimalHidden, DEFAULTS.skeets.minimalHidden),
      backoffStartMs: parseMs(clientConfig?.polling?.backoffStartMs, DEFAULTS.backoffStartMs),
      backoffMaxMs: parseMs(clientConfig?.polling?.backoffMaxMs, DEFAULTS.backoffMaxMs),
      jitterRatio: (() => {
        const n = Number(clientConfig?.polling?.jitterRatio);
        return Number.isFinite(n) && n >= 0 && n <= 1 ? n : DEFAULTS.jitterRatio;
      })(),
      heartbeatMs: parseMs(clientConfig?.polling?.heartbeatMs, DEFAULTS.heartbeatMs),
    });
  ...
  const refreshNow = useCallback(async (opts) => {
    if (pollingRef.current && typeof pollingRef.current.triggerNow === "function") {
        ...
        setSkeets((current) =>
          current.map((s) =>
            s.id === id
              ? {
                  ...s,
                  likesCount: typeof total.likes === "number" ? total.likes : s.likesCount,
                  repostsCount: typeof total.reposts === "number" ? total.reposts : s.repostsCount,
                }
              : s
          )
        );
