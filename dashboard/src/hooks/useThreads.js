...
function shouldKeepPolling(thread) {
  ...
  // setAutoRefresh() bleibt damit false – ein Reload passiert nur manuell
  // oder wenn der Statuswechsel separat erkannt wird.
  void thread;
  return false;
}
...
export function useThreads(options = {}) {
  ...
  useEffect(() => {
    ...
    const controller = createPollingController({
      fetchBluesky: async () => {
        ...
        if (!res.ok) {
          ...
          if (res.status === 429 || res.status === 503) {
            ...
            const err = new Error(errorData.error || "Rate Limit erreicht – später erneut versuchen.");
      ...
      activeIntervalMs: parseMs(clientConfig?.polling?.threads?.activeMs, DEFAULTS.threads.activeMs),
      idleIntervalMs: parseMs(clientConfig?.polling?.threads?.idleMs, DEFAULTS.threads.idleMs),
      hiddenIntervalMs: parseMs(clientConfig?.polling?.threads?.hiddenMs, DEFAULTS.threads.hiddenMs),
      minimalPingWhenHidden: parseBool(clientConfig?.polling?.threads?.minimalHidden, DEFAULTS.threads.minimalHidden),
