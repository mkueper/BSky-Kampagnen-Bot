# Cleanup-Analyse BSky-Kampagnen-Tool – 2025-12-02
...
- Aktuell gibt es keine identifizierten Dashboard-spezifischen Cleanup-Kandidaten; der früher dort angesiedelte `useThemeDebug`-Hook wurde entfernt und lebt jetzt als `packages/shared-ui/src/hooks/useThemeDebug.js` (siehe Shared-UI).
...
- Der früher erwähnte Re-Export `bsky-client/src/hooks/useThemeMode.js` existiert nicht mehr; `BskyClientLayout` und andere Module importieren `useThemeMode` direkt aus `@bsky-kampagnen-bot/shared-ui`. Damit entfällt der vorherige Cleanup-Hinweis.
...
- `packages/shared-ui/src/utils/compressImage.js`  
  ...
  **Begründung:**  
  - Wird aktuell in `packages/shared-ui/src/components/MediaDialog.jsx` eingesetzt, um aufgenommene Medien vor dem Upload zu komprimieren.  
  - Der Utility-Import ist damit produktiv und sollte nicht gelöscht; falls die Kompression in ein spezielleres Paket wandert (z. B. `shared-logic`), muss die Abhängigkeit zuerst angepasst werden.
  - Empfehlung: Datei behalten, gegebenenfalls erst nach Migration verschieben.

- `packages/shared-ui/src/hooks/useThemeDebug.js`  
  **Typ:** Hook (Debug)  
  **Kategorie:** ✅ sicher löschbar  
  **Begründung:**  
  - `useThemeDebug` liefert nur Logging und wird in keiner Komponente oder Hook innerhalb des Repos importiert.  
  - Die einzige Referenz ist die Hilfsfunktion selbst, sodass sich der Hook ohne Seiteneffekte entfernen oder in ein `legacy/`-Verzeichnis verschieben lässt.
...
- Hooks: `useToast.js`, `useThemeMode.js`, `useThemeDebug.js` – `useToast` und `useThemeMode` sind produktiv im Einsatz; `useThemeDebug` ist aktuell nirgends importiert und dient nur noch als temporärer Debug-Helper.
...
1. **Batch 1 – sehr konservativ:**  
   - Optional: `packages/shared-ui/src/hooks/useThemeDebug.js` in ein `legacy/`-Verzeichnis verschieben oder löschen; `compressImage.js` wird hingegen in `MediaDialog` verwendet und darf nicht ohne Migration entfernt werden.  
   - Danach: `npm test`, `npm run lint`, ggf. `npm run build:frontend` laufen lassen.

2. **Batch 2 – Stil-Aufräumung:**  
   - Re-Export `bsky-client/src/hooks/useThemeMode.js` wurde entfernt; `useThemeMode` wird durchgängig direkt aus `@bsky-kampagnen-bot/shared-ui` importiert, daher entfällt die eigentliche Aufgabe.
